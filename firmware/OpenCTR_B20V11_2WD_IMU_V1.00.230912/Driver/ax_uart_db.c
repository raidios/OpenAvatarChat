		...
		else    //==接收数据
		{
			...
			else    //判断最后1位
			{
				...
				{	
					...
					//运动控制帧
					if(uart_db_rx_buf[3] == 0x02)
					{
						RobotV_TG.I_X = (int16_t)((uart_db_rx_buf[4]<<8) | uart_db_rx_buf[5]);
						RobotV_TG.I_Y = (int16_t)((uart_db_rx_buf[6]<<8) | uart_db_rx_buf[7]);
						RobotV_TG.I_W = (int16_t)((uart_db_rx_buf[8]<<8) | uart_db_rx_buf[9]);
					}
					
					//PID调试
					if(uart_db_rx_buf[3] == 0x03)
					{
						ax_motor_kp = (int16_t)((uart_db_rx_buf[4]<<8) | uart_db_rx_buf[5]);
						ax_motor_kd = (int16_t)((uart_db_rx_buf[6]<<8) | uart_db_rx_buf[7]);
					}
					
					//舵机零位偏置角
					if(uart_db_rx_buf[3] == 0x04)
					{
						ax_servo_offset = (int16_t)((uart_db_rx_buf[4]<<8) | uart_db_rx_buf[5]);
					}
				}
			}
		}
